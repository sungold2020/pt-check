...
RSSList=[\
    {'Name':'BeiTai',   'Url':'https://www.beitai.pt/torrentrss.php?rows=20&icat=1&ismalldescr=1&isize=1&linktype=dl&passkey=&inclbookmarked=1'},\
    {'Name':'MTeam',    'Url':'https://pt.m-team.cc/torrentrss.php?https=1&rows=20&icat=1&ismalldescr=1&isize=1&linktype=dl&passkey=&inclbookmarked=1'}]
        ...
        if gIsNewDay == True :   #新的一天，更新记录每天的上传量（绝对值）
            ...
            if IsLowUpload(gTorrentList[tNoOfTheList].DateData) :
                if self.Status != STOP and \
                   gTorrentList[tNoOfTheList].Client == QB and \
                   gTorrentList[tNoOfTheList].Category == '下载': 
                    return LOWUPLOAD
            return UPDATED
...
def SaveTorrent(torrent):
    ...
    #1、从rss表中获取toubanid和imdbid
    HASH = torrent.hash
    g_DB = mysql.connector.connect( host="localhost",  user="dummy",  passwd="moonbeam" , database="db_movies")
    g_MyCursor = g_DB.cursor()
    sel_sql = 'select DoubanID,IMDBID,Type,Name,Nation,Director,Actors from rss where id = %s'
    sel_val = (HASH,)
    g_MyCursor.execute(sel_sql,sel_val)
    SelectResult = g_MyCursor.fetchall()
    if len(SelectResult) != 1: ErrorLog("failed to find select from rss :{}".format(torrent.name)); return False
    ...
    if tMovieInfo["success"]: 
        ...
        ErrorLog("failed to request from douban:"+torrent.name)
...
def ReadPTBackup():
    ...
    if not os.path.isfile(TorrentListBackup):
        ExecLog(TorrentListBackup+" does not exist")
        return 0
    ...
    for torrent in torrents:
        if torrent.category == '下载' or torrent.category == '刷上传' or torrent.category == '保种' : 
            try:
                ...
                ExecLog("failed to resume:"+torrent.name)
    return True
...
def TrackerData():
    ...
    for i in range(len(TrackerDataList)):
        ExecLog( "{} upload(M):{}".format((TrackerDataList[i]['Name']).zfill(10),str(TrackerDataList[i]['DateData'][-1]['Data']/(1024*1024))) )
    ...
    for i in range(len(TrackerDataList)):
        ...
        while j >= 0 :
            ...
            j -= 1
        ExecLog( "{} {} days no upload".format(TrackerDataList[i]['Name'].zfill(10),str(NumberOfDays).zfill(2)) )
...
def HandleRequest(tRequest):
    Reply = "success"
    return  bytes(Reply,encoding='utf-8')
...
def ReadRSSTorrentBackup():
    ...
    if not os.path.isfile(RSSTorrentBackupFile):
        ExecLog(RSSTorrentBackupFile+" does not exist")
        return 0
...
def WriteRSSTorrentBackup():
    ...
    try :
        fo = open(RSSTorrentBackupFile,"w")
    except:
        ErrorLog("Error:open backup file to write："+RSSTorrentBackupFile)
        return -1
        
    i = 0; tLength = len(gRSSTorrentList)
    while i < tLength :
...
def RSSTask(tRSSName=None):
    ...
    #进行RSS订阅
    if tRSSName == None :
        for tRSS in RSSList:  RSSRequest(tRSS)
    else:
        for tRSS in RSSList : 
            if tRSSName.lower() == tRSS['Name'].lower() : RSSRequest(tRSS)
...
def RSSRequest(tRSS):
    ...
    for i in range(len(items)):
        ...
        DoubanScore = DoubanID = DoubanLink = IMDBLink = IMDBScore = IMDBID = ""
        ...
        if RSSName == "LeagueHD" or\
           ...
           RSSName == "MTeam"    :
            ...
            if tIndex >= 0 :
                ...
                tIndex = Nation.find('\n')
                Nation = Nation[:tIndex]
                Nation = Nation.strip()
                Nation = Nation[ :Nation.find('<') ]
        ...
        in_sql = "INSERT INTO rss \
                (RSSName, ID, Title, DownloadLink, DoubanScore, DoubanLink, DoubanID, IMDBScore, IMDBLink, IMDBID, Type, Nation, Name, Director, AddDate) \
          VALUES(%s,      %s, %s   , %s          , %s         , %s        , %s      , %s       , %s      , %s    , %s  , %s    , %s  , %s      , %s  )" 
        in_val= (RSSName, ID, Title, DownloadLink, DoubanScore, DoubanLink, DoubanID, IMDBScore, IMDBLink, IMDBID, Type, Nation, Name, Director, AddDate)
